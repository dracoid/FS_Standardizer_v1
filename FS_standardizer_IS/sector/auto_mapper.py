...
def classify_auto(tag, segments, plabel):
    ...
    # Step 2: Subcategory fallback (if not set above)
    if 'subcategory' not in locals() or subcategory is None:
        if "automotiveleasing" in seg_lower:
            subcategory = "Automotive Leasing"
        elif "automotivesales" in seg_lower or "automobiles" in seg_lower:
            subcategory = "Automotive Sales"
        ...
        elif "regulatorycredit" in seg_lower or "regulatory" in seg_lower:
            subcategory = "Regulatory Credit"
        else:
            subcategory = "Other"
